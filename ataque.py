...
def parse_credentials(input_string):
    if ':' in input_string and not input_string.startswith('http'):
        ...
        if len(parts) == 2:
            ...
            print(Fore.RED + f"\nEl userpass '{input_string}' no tiene el formato correcto" + Style.RESET_ALL)
            return None, None
...
def run_espejos():
    ...
    def send_to_telegram_espejos(valid_servers, base_filename):
        ...
        if send_to_telegram(bot_token, chat_id, message):
            print("\n" + Fore.GREEN + "Resultado enviado a Telegram correctamente." + Style.RESET_ALL)
            ...
            try:
                ...
                if choice == '1':
                    ...
                    while True:
                        ...
                        if archivo_seleccionado or archivos_seleccionados:
                            files_to_process = archivos_seleccionados if archivos_seleccionados else [archivo_seleccionado]
                            for cred_file_path in files_to_process:
                                if os.path.exists(cred_file_path):  # Verificar si el archivo existe
                                    with open(cred_file_path, 'r') as cred_file:
                                        for line in cred_file:
                                            ...
                                            username, password = parse_credentials(line)
                                            if username and password:
                                                credentials.append(f"{username}:{password}")
                            break
                        else:
                ...
                for urls_file in urls_files:
                    if os.path.exists(urls_file):  # Verificar si el archivo existe antes de abrir
                        ...
                        base_filename = input("\nIngresa el nombre del server que se enviará a Telegram: ")
                        ...
                        if valid_servers:
                            ...
                            # Procesar servidores válidos y almacenar en archivo si es necesario
                            with open(espejos_file_path, 'w') as espejos_file:
                                for server, details in valid_servers.items():
                                    # Supongamos que `server` tiene un formato como 'http://smartersdance.com:8080/path?username=user&password=pass'
                                    
                                    # Eliminar el protocolo y cualquier ruta adicional
                                    clean_server = server.split("//")[-1]  # Elimina el protocolo (http:// o https://)
                            
                                    # Extraer solo el nombre del servidor y el puerto
                                    server_name_port = clean_server.split('/')[0]  # Obtiene 'smartersdance.com:8080'
                            
                                    # Guardar en el archivo
                                    espejos_file.write(f"{server_name_port}\n")
