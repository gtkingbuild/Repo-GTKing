...
def run_speed_test():
    ...
    try:
        ...
        logging.log("[Speed Test] Error al Ejecutar el Test de Velocidad: {0}".format(e), level=xbmc.LOGDEBUG)
        pass
...
def system_info():
    ...
    directory.add_file('[COLOR {0}]Tiempo Actual:[/COLOR] [COLOR {1}]{2}[/COLOR]'.format(CONFIG.COLOR1, CONFIG.COLOR2, data[6]), icon=CONFIG.ICONMAINT, themeit=CONFIG.THEME2)
    directory.add_file('[COLOR {0}]Tiempo Total:[/COLOR] [COLOR {1}]{2}[/COLOR]'.format(CONFIG.COLOR1, CONFIG.COLOR2, data[7]), icon=CONFIG.ICONMAINT, themeit=CONFIG.THEME2)
    ...
    directory.add_file('[COLOR {0}]Programa Addons:[/COLOR] [COLOR {1}]{2}[/COLOR]'.format(CONFIG.COLOR1, CONFIG.COLOR2, str(len(programs))), icon=CONFIG.ICONMAINT, themeit=CONFIG.THEME2)
    directory.add_file('[COLOR {0}]Musica Addons:[/COLOR] [COLOR {1}]{2}[/COLOR]'.format(CONFIG.COLOR1, CONFIG.COLOR2, str(len(music))), icon=CONFIG.ICONMAINT, themeit=CONFIG.THEME2)
    directory.add_file('[COLOR {0}]Imagenes Addons:[/COLOR] [COLOR {1}]{2}[/COLOR]'.format(CONFIG.COLOR1, CONFIG.COLOR2, str(len(picture))), icon=CONFIG.ICONMAINT, themeit=CONFIG.THEME2)
...
def save_menu():
    ...
    directory.add_file('- Haga Clic para Alternar la Configuración -', themeit=CONFIG.THEME3)
...
def enable_addons():
    ...
    directory.add_file("[I][B][COLOR red]!!Aviso: Deshabilitar Algunos Addons Puede Causar Problemas!![/COLOR][/B][/I]", icon=CONFIG.ICONMAINT)
    fold = glob.glob(os.path.join(CONFIG.ADDONS, '*/'))
    addonnames = []
    addonids = []
    for folder in sorted(fold, key=lambda x: x):
...
def change_freq():
    ...
    change = dialog.select("[COLOR {0}]Con qué frecuencia usarias la limpieza Automática al Iniciar?[/COLOR]".format(CONFIG.COLOR2), CONFIG.CLEANFREQ)
    if not change == -1:
