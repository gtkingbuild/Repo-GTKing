	...
	if not WORKINGURL == True:
		...
		if len(match) >= 1:
			if SEPERATE == 'true':
				...
				if count16 > 0:
					state = '[COLOR red]+[/COLOR]' if SHOW16 == 'false' else '[COLOR red]-[/COLOR]'
					addFile('[B]%s [COLOR red]Instalar MOTOR (%s)[/COLOR][/B]' % (state, count16), 'togglesetting',  'show16', themeit=THEME3)
					if SHOW16 == 'false':
						for name, version, url, gui, kodi, theme, icon, fanart, adult, description in match:
							if not SHOWADULT == 'true' and adult.lower() == 'yes': continue
							if not DEVELOPER == 'true' and wiz.strTest(name): continue
				...
				if count15 > 0:
					state = '[COLOR dodgerblue]+[/COLOR]' if SHOW15 == 'false' else '[COLOR dodgerblue]-[/COLOR]'
					addFile('[B]%s [COLOR dodgerblue]Parches GTKING (%s)[/COLOR][/B]' % (state, count15), 'togglesetting',  'show15', themeit=THEME2)
					if SHOW15 == 'false':
						for name, version, url, gui, kodi, theme, icon, fanart, adult, description in match:
							if not SHOWADULT == 'true' and adult.lower() == 'yes': continue
							if not DEVELOPER == 'true' and wiz.strTest(name): continue
...
def viewBuild(name):
    ...
    for version, url, gui, kodi, themefile, icon, fanart, preview, adult, info, description in match:
        ...
        if warning == True:
            addFile('[COLOR white][B]INSTALAR:[/B][/COLOR] [COLOR red][B]MOTOR[/B][/COLOR] / [COLOR dodgerblue][B]PARCHE[/B][/COLOR]', 'install', name, 'normal' , description=description, fanart=fanart, icon=icon, themeit=THEME1)
