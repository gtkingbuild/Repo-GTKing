...
class Wizard:
    ...
    def build(self, name, over=False):
        ...
        if warning:
            yes_pressed = self.dialog.yesno("{0} - [COLOR red]ADVERTENCIA!![/COLOR]".format(CONFIG.ADDONTITLE), '[COLOR {0}]Existe la posibilidad de que la skin no se vea correctamente'.format(CONFIG.COLOR2) + '\n' + 'Al instalar un {0} build en un Kodi {1} Instalar'.format(check.check_build(name, 'kodi'), CONFIG.KODIV) + '\n' + 'Todavía te gustaría instalar: [COLOR {0}]{1} v{2}[/COLOR]?[/COLOR]'.format(CONFIG.COLOR1, name, check.check_build(name, 'version')), nolabel='[B][COLOR red]No, Cancelar[/COLOR][/B]', yeslabel='[B][COLOR springgreen]Si, Instalar[/COLOR][/B]')
        else:
            if over:
                yes_pressed = 1
            else:
        ...
        if yes_pressed:
            ...
            if int(float(percent)) > 0:
                db.fix_metas()
                CONFIG.set_setting('build name', name)
                CONFIG.set_setting('build version', check.check_build(name, 'version'))
                CONFIG.set_setting('build theme', '')
                CONFIG.set_setting('latest version', check.check_build(name, 'version'))
                CONFIG.set_setting('nex tbuild check', tools.get_date(days=CONFIG.UPDATECHECK, formatted=True))
                ...
                if int(float(errors)) > 0:
                    ...
                    if yes_pressed:
                        ...
                        window.show_text_box("Visualización de Errores de Instalación de Build ", error)
    ...
    def gui(self, name, over=False):
        if name == CONFIG.get_setting('build name'):
            if over:
                yes_pressed = 1
            else:
                yes_pressed = self.dialog.yesno(CONFIG.ADDONTITLE,
                                   '[COLOR {0}]Le gustaría aplicar el guifix para:'.format(CONFIG.COLOR2) + '\n' + '[COLOR {0}]{1}[/COLOR]?[/COLOR]'.format(CONFIG.COLOR1, name),
        ...
        if yes_pressed:
            ...
            logging.log_notify(CONFIG.ADDONTITLE,
                               '[COLOR {0}]GuiFix: Cancelada![/COLOR]'.format(CONFIG.COLOR2))
