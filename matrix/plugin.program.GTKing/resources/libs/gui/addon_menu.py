...
class AddonMenu:
    ...
    def show_menu(self, url=None):
        ...
        if url_response:
            ...
            TEMPADDONFILE = None
            logging.log("[Addon Menu] No se agregó ninguna lista de complementos.")
            ...
            if addons_json:
                ...
                if addons and len(addons) > 0:
                    ...
                    if not addons:
                        ...
                        directory.add_file("Aún no se agregaron complementos a este menú!", themeit=CONFIG.THEME2)
        else:
    ...
    def install_addon_from_url(self, plugin, url):
        ...
        if not response:
            logging.log_notify("[COLOR {0}]Addon Instalador[/COLOR]".format(CONFIG.COLOR1),
        ...
        title = '[COLOR {0}][B]Instalando:[/B][/COLOR] [COLOR {1}]{2}[/COLOR]'.format(CONFIG.COLOR2, CONFIG.COLOR1,
                                                                                      plugin)
        self.progress_dialog.update(0, title, '', '[COLOR {0}]Espere por favor[/COLOR]'.format(CONFIG.COLOR2))
    ...
    def install_addon_pack(self, name, url):
        ...
        if not response:
            logging.log_notify("[COLOR {0}]Addon Instalador[/COLOR]".format(CONFIG.COLOR1),
                               '[COLOR {0}]{1}:[/COLOR] [COLOR {2}]Url Invalido Zip![/COLOR]'.format(CONFIG.COLOR1, name, CONFIG.COLOR2))
            return
        ...
        progress_dialog.update(0, title, '', '[COLOR {0}]Espere por favor[/COLOR]'.format(CONFIG.COLOR2))
        percent, errors, error = extract.all(lib, CONFIG.ADDONS, title=title)
