...
def get_videos_m3u8(url, video=Video(), fn_url=None):
    ...
    if '.m3u8' in url:
        ...
        try:
            ...
            for stream_if, file_uri in re.findall(r'#EXT-X-STREAM-INF:(.*?)\n(.*?)\n', m3u8_data + r'\n', re.I):
                res = 0
                stream_if = stream_if.replace('"', "'")
                comillas = re.findall(r"'(.*?)'", stream_if)

                for n, v in  enumerate(comillas):
                    stream_if= stream_if.replace(v, "@@%s@@" %n, 1)

                stream = dict()
                for it in stream_if.split(','):
                    key = it.split('=')[0].upper()
                    value = it.split('=')[1]
                    index = re.findall(r"@@(\d+)@@", value)
                    if index:
                        value = re.sub(r"'@@\d+@@'", comillas[int(index[0])], value)
                    stream[key]= value.strip()

                try:
                    res = stream.pop('RESOLUTION')
                ...
                if fn_url:
                    ...
                    #logger(file_uri)
                elif not file_uri.startswith('http'):
                    url_base, name = url.split('.m3u8')[0].rsplit('/', 1)
                    name += '.m3u8'
                    if '?' in file_uri:
                        file_uri = url_base + '/' + file_uri
                    else:
                        file_uri = url.replace(name,file_uri)
                    #logger(file_uri)

                new_video = video.clone(url=file_uri.strip())
                if res:
                    new_video.res = res
                if stream.get('BANDWIDTH'):
                    new_video.bitrate = stream.pop('BANDWIDTH')
                new_video.__dict__.update(stream)
    ...
    if len(video_list) > 1:
        new_video = video.clone(res=res_max, url=url, bitrate='multi-bitrate', type='hls')
        video_list.insert(0, new_video)

    elif not video_list and m3u8_code == 200:
