...
def whitelist(do):
    ...
    if do == 'edit':
        ...
        while choice not in [-1, 0]:
            choice = dialog.select("{0}: Seleccione los add-ons que desea incluir en la whitelist.".format(CONFIG.ADDONTITLE), tempaddonnames)
            if choice == -1:
                break
            elif choice == 0:
                break
        ...
        if len(list) > 0:
            msg = "Aquí hay una lista de los elementos de su whitelist, estos elementos (junto con las dependencias) no se eliminarán cuando se realice un nuevo comienzo o los datos de usuario se sobrescriban en una instalación de la build.[CR][CR]"
            for item in list:
                try:
                    name, id, fold = item
                except Exception as e:
            ...
            window.show_text_box("Visualización de elementos de la Whitelisted", msg)
        else:
            logging.log_notify(CONFIG.ADDONTITLE,
                               "[COLOR {0}]No hay elementos en la  whitelist[/COLOR]".format(CONFIG.COLOR2))
    elif do == 'import':
        source = dialog.browse(1, '[COLOR {0}]Seleccione el archivo de la whitelist para importar[/COLOR]'.format(CONFIG.COLOR2),
        ...
        source = dialog.browse(3,
                                   '[COLOR {0}]Seleccione dónde desea exportar el archivo del whitelist[/COLOR]'.format(CONFIG.COLOR2),
                                   'files', '.txt', False, False, CONFIG.HOME)
        logging.log(str(source))
        try:
